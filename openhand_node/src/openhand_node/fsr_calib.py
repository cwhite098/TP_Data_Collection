import time
import pandas as pd
from hardware import FSR, FTsensor
...
def main():
    ...
    df = pd.DataFrame(columns=['FSR_Reading', 'Forcex', 'Forcey', 'Forcez'])
    ...
    for i in range(no_cycles):
        ...
        for j in range(readings_per_cycle):
            ...
            # add to dataframe
            if (fsr_reading != 0) and (-ft_readings[2] <10):
                df2 = pd.DataFrame([[fsr_reading, ft_readings[0], ft_readings[1], -ft_readings[2]]],
                columns=['FSR_Reading', 'Forcex', 'Forcey', 'Forcez'])
                df = df.append(df2)
    ...
    plt.scatter(df['FSR_Reading'], df['Forcez'])
    plt.show()
